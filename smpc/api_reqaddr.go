...
func (req *ReqSmpcAddr) DoReq(raw string, workid int, sender string, ch chan interface{}) bool {
	...
	if ok {
		...
		common.Info("===================DoReq the pubkey requestion====================", "key ", key)
		...
		common.Info("===================DoReq,set nonce ok====================", "key ", key)
		...
		common.Info("===================DoReq,get group sigs ok====================", "key ", key)
		...
		if req2.Mode == "0" { // self-group
			...
			go func(wid int) {
				...
				common.Info("==================Waiting For Accept====================", "key ", key)
				wtmp2 := workers[wid]
				for {
					select {
					case account := <-wtmp2.acceptReqAddrChan:
						...
						for _, nr := range ars {
							if !strings.EqualFold(nr.Status, "Agree") {
								common.Error("==================nr.Status====================", "status ", nr.Status)
								reply = false
								break
							}
						}
		...
		common.Info("===================Do smpcGenPubKey====================", "key ", key)
